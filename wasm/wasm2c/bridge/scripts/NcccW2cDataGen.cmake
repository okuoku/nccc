...
# To support Android studio CMake 3.10
function(decodehexdigit out hexchar)
    if(${hexchar} MATCHES "[0-9]")
        set(r ${hexchar})
    elseif(${hexchar} MATCHES "A")
        set(r 10)
    elseif(${hexchar} MATCHES "B")
        set(r 11)
    elseif(${hexchar} MATCHES "C")
        set(r 12)
    elseif(${hexchar} MATCHES "D")
        set(r 13)
    elseif(${hexchar} MATCHES "E")
        set(r 14)
    elseif(${hexchar} MATCHES "F")
        set(r 15)
    else()
        message(FATAL_ERROR "Unknown hexdigit ${hexchar}")
    endif()
    set(${out} ${r} PARENT_SCOPE)
endfunction()

function(decodehex out hexstr)
    if(${hexstr} MATCHES "^(.)(.)")
        decodehexdigit(c1 ${CMAKE_MATCH_1})
        decodehexdigit(c2 ${CMAKE_MATCH_2})
        math(EXPR acc "${c1}*16+${c2}")
    else()
        message(FATAL_ERROR "Invalid input for decodehex ${hexstr}")
    endif()
...
function(decodename out nam)
    ...
    while(1)
        if("${q}" MATCHES "^([^Z]*)Z([0-9A-F][0-9A-F])(.*)")
            ...
            decodehex(dec ${hex})
            #message(STATUS "Decodehex: ${hex} => ${dec}")
            string(ASCII ${dec} c)
